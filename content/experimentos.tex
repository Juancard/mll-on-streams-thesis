...
precisión (mejor clasificador) pero un \textit{recall} por debajo del 20\%
(segundo peor clasificador). Mismo caso pero a la inversa con \acrshort{dwm}
(\acrshort{br}) para Mediamill, el cual consigue un 40\% de \textit{recall}
(tercer mejor clasificador) pero apenas un 0.006\% de precisión (tercer peor
clasificador).  Esta imparidad logra suavizarse en los modelos de
\acrshort{efmp} presentados y se ve reflejado en mejores valores de \textit{f1}.
